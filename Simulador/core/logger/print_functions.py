...
def print_debug_raiz_x1n(i, j, V, R, AymaxL, x1N_bruto=None, x1N_ajustado=None):
    ...
    if x1N_bruto is None or x1N_ajustado is None:
        ...
        if isinstance(x1N_bruto, complex) or isinstance(x1N_ajustado, complex):
            print(f"ℹ️ Comparação entre complexos desativada — ajuste feito manualmente por fallback.")
        else:
            if abs(x1N_bruto - x1N_ajustado) < 1e-6:
                ...
                print(f"Motivo do ajuste                  : Valor modificado por outro critério")
